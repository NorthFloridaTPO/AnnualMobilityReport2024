...
df_lottr <- read.csv("~/GitHub/AnnualMobilityReport2024/data/LOTTR.csv", check.names=FALSE, row.names=1) 
...
# Create hover-over labels 
labs <- paste0("<b>Trail Name: </b>", geojson_data$Name, "<br><b>County: </b>", geojson_data$County, "<br><b>Scale: </b>", geojson_data$Scale)
...
leaflet() %>%
  ...
  addProviderTiles(providers$CartoDB.Positron) %>%
  setView(lng = -81.655651, lat = 30.232184, zoom = 9)%>%
  addPolylines(data = geojson_data, color = ~pal(Scale), popup = ~paste(labs), opacity = 1.0, label = ~lapply(labs, htmltools::HTML))%>%
  addLegend("topright", 
